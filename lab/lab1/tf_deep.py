...
class TFDeep:
    def __init__(self, layers, param_delta=0.5, param_lambda=1e-3, activation=tf.nn.relu):
        ...
        for i in range(1, len(layers)):
            self.w.append(tf.get_variable("W_%d" % i, shape=(layers[i - 1], layers[i]),
                                          initializer=tf.random_normal_initializer(stddev=0.1)))
            self.b.append(
                tf.get_variable("b_%d" % i, shape=(layers[i],), initializer=tf.random_normal_initializer(stddev=0.1)))
        ...
        learning_rate = tf.train.exponential_decay(param_delta, global_step=global_step, decay_steps=1,
                                                   decay_rate=1 - 1e-4)
    ...
    def train_mb(self, X, Yoh_, param_niter, n_batches):
        ...
        for i in range(param_niter):
            ...
            for j in range(n_batches):
                _, l = self.session.run([self.train_step, self.loss],
                                        feed_dict={self.X: batch_X[j], self.Y: batch_Y[j]})
            if i % 1000 == 0:
                print("Loss at %d: %f" % (i, l))
