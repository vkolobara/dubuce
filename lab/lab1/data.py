...
def eval_perf_binary(Y, Y_):
    tp = sum(np.logical_and(Y == Y_, Y_ is True))
    fn = sum(np.logical_and(Y != Y_, Y_ is True))
    tn = sum(np.logical_and(Y == Y_, Y_ is False))
    fp = sum(np.logical_and(Y != Y_, Y_ is False))
    recall = tp / (tp + fn)
    precision = tp / (tp + fp)
